...
v_1 = pi_1 / (2*pi * sigma_1 * sigma_2) *...
    exp(-(x_donnees_centrees_1.^2 + y_donnees_centrees_1.^2)/(2 * sigma_1 * sigma_2));
v_2 = pi_2 / (2*pi * sigma_1 * sigma_2) *...
    exp(-(x_donnees_centrees_2.^2 + y_donnees_centrees_2.^2)/(2 * sigma_1 * sigma_2));
...
vraisemblances_classe_1 = v_1(indice_max, :);
vraisemblances_classe_2 = v_2(indice_max, :);
...
n_1 = length(classe_1);
pi_1 = n_1/n;
x_mu_1 = mean(x_classe_1);
x_classe_1_centre = x_classe_1-x_mu_1;
y_mu_1 = mean(y_classe_1);
y_classe_1_centre = y_classe_1-y_mu_1;
Sigma_1 = diag([sum(x_classe_1_centre.^2) sum(y_classe_1_centre.^2)]) / n_1;

% Estimation des parametres de la loi normale associee a la deuxieme classe :
n_2 = length(classe_2);
pi_2 = n_2/n;
x_mu_2 = mean(x_classe_2);
x_classe_2_centre = x_classe_2-x_mu_2;
y_mu_2 = mean(y_classe_2);
y_classe_2_centre = y_classe_2-y_mu_2;
Sigma_2 = diag([sum(x_classe_2_centre.^2) sum(y_classe_2_centre.^2)]) / n_2;
...
while abs(n_1-n_1_precedent)/n_1>seuil
	...
	vraisemblances_classe_1 = pi_1 / (2*pi*sqrt(a_1*c_1-b_1^2)) *...
        exp(- (a_1 * x_donnees_centrees_1.^2 + 2 * b_1 * x_donnees_centrees_1 .* y_donnees_centrees_1 + c_1 * y_donnees_centrees_1.^2) / 2);
    vraisemblances_classe_1 = vraisemblances_classe_1 / norm(vraisemblances_classe_1);
	...
	vraisemblances_classe_2 = pi_2 / (2*pi*sqrt(a_2*c_2-b_2^2)) *...
        exp(- (a_2 * x_donnees_centrees_2.^2 + 2 * b_2 * x_donnees_centrees_2 .* y_donnees_centrees_2 + c_2 * y_donnees_centrees_2.^2) / 2);
    vraisemblances_classe_2 = vraisemblances_classe_2 / norm(vraisemblances_classe_2);
	...
	y_mu_1 = mean(y_classe_1);
	y_classe_1_centre = y_classe_1-y_mu_1;
	Sigma_1 = [sum(x_classe_1_centre.^2) sum(x_classe_1_centre .* y_classe_1_centre);...
        sum(x_classe_1_centre .* y_classe_1_centre) sum(y_classe_1_centre.^2)] / n_1;

	% Estimation des parametres de la loi normale associee a la deuxieme classe :
	n_2 = length(classe_2);
	pi_2 = n_2/n;
	x_mu_2 = mean(x_classe_1);
	x_classe_2_centre = x_classe_2-x_mu_2;
	y_mu_2 = mean(y_classe_2);
	y_classe_2_centre = y_classe_2-y_mu_2;
	Sigma_1 = [sum(x_classe_2_centre.^2) sum(x_classe_2_centre .* y_classe_2_centre);...
        sum(x_classe_2_centre .* y_classe_2_centre) sum(y_classe_2_centre.^2)] / n_2;
end
