...
[Fils, weights]= Modification(weights,Parents,im,r,niter,coefs,knots,subdiv1,cseuil,p_mut);
disp('  MAJ reussie..');
%Test premier critere d'arrêt: Test si la popoulation nevolue plus
% if it>=6
...
%% results
 ...
 if (Eval(indice_elu).nb_CO==0 && Eval(indice_elu).nb_CC==0)
      sol=1;
      disp(['La solution est determinee à l iteration numero:  ' num2str(it-1)]);
...
% 
% colormap(jet);
% plot(coefs(1,:),coefs(2,:),'*r');%% les sommets de la courbe à approximer (formant le polygone de controle)
% hold on
% 
% colormap(jet);
