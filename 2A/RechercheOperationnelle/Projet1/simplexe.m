...
%
function [x,F,GAMMA,JN] = simplexe(OPT,c,A,b)
    disp('call to simplexe');
%
%% Fonction simplexe
%
% Le pb : Opt F(x) = c'*x
...
A = A.*repmat(s, 1, n); % utiliser la fonction repmat !
b = s.*b;
%
%% PHASE I
% Trouver un sommet initial
if all(all(A(:,n-m+1:n) == eye(m)))
    disp('  case ineq');
    ...
    disp('  case eq');
    Ab = [A eye(m, m)];
    cb = [zeros(1, n) ones(1, m)];
    bb = b;
    [xb,Fb,GAMMAb,JNb] = simplexe(-1,cb,Ab,bb);
    disp('  base found');
    JN = setdiff(JNb, (n+1:n+1+m));
    JB = setdiff(J, JN);
    A = GAMMAb(:,1:n)
    b = GAMMAb(:,(n+1+m))
    GAMMA = [A b];
...
while any(OPT*pentes > 0) % Y-a-t-il un chemin améliorant ?
    ...
    GAMMAMOINS = GAMMAMOINS - diag(GAMMAMOINS(:,alpha)) * (ones(m-1,1) * GAMMA(indbeta,:));
