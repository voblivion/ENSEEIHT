...
% Calcul des probabilité à posteriori
VAP_pensees = 0.05;
VAP_oeillets = 0.05;
VAP_chrysanthemes = 1 - VAP_pensees - VAP_oeillets;
MAP_pensees = vraisemblance_pensees * VAP_pensees;
MAP_oeillets = vraisemblance_oeillets * VAP_oeillets;
MAP_chrysanthemes = vraisemblance_chrysanthemes * VAP_chrysanthemes;

% Calcul du maximum de vraisemblance :
MAP = max(MAP_pensees, MAP_oeillets);
MAP = max(MAP,MAP_chrysanthemes);
classe_pensees = find(MAP_pensees == MAP);
classe_oeillets = find(MAP_oeillets == MAP);
classe_chrysanthemes = find(MAP_chrysanthemes == MAP);
