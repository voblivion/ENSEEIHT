...
nb_valeurs_donnees_aberrantes = length(valeurs_donnees_aberrantes);
ecarts_angulaires_D_perp_1 = zeros(nb_essais,nb_valeurs_donnees_aberrantes);
ecarts_angulaires_D_perp_2 = zeros(nb_essais,nb_valeurs_donnees_aberrantes);
ecarts_angulaires_D_perp_3 = zeros(nb_essais,nb_valeurs_donnees_aberrantes);
for i = 1:nb_essais
	...
	for j = 1:nb_valeurs_donnees_aberrantes
		...
		arguments_perp = sum(log(residus_perp.*residus_perp+beta*beta)-2*log(beta), 2);
		...
		ecarts_angulaires_D_perp_3(i,j) = ecart_angulaire_perp;

		% Droite D_perp estimee par MV :
        alpha = 0;
		residus_perp = cos_theta*x_donnees_centrees+sin_theta*y_donnees_centrees;
		arguments_perp = sum(sqrt(residus_perp.*residus_perp+alpha*alpha)-alpha, 2);
		[minimum_perp,indice_min_perp] = min(arguments_perp);
		theta_perp = valeurs_theta(indice_min_perp);
		ecart_angulaire_perp = min(abs(theta_perp-theta_0),abs(theta_perp-theta_0+pi));
		ecart_angulaire_perp = min(ecart_angulaire_perp,abs(theta_perp-theta_0-pi));
...
moyennes_ecarts_angulaires_D_perp_1 = mean(ecarts_angulaires_D_perp_1)/pi*180;
moyennes_ecarts_angulaires_D_perp_2 = mean(ecarts_angulaires_D_perp_2)/pi*180;
moyennes_ecarts_angulaires_D_perp_3 = mean(ecarts_angulaires_D_perp_3)/pi*180;
...
plot(100*valeurs_donnees_aberrantes/nb_donnees,moyennes_ecarts_angulaires_D_perp_1,'g-','LineWidth',2);
plot(100*valeurs_donnees_aberrantes/nb_donnees,moyennes_ecarts_angulaires_D_perp_2,'b-.','LineWidth',2);
plot(100*valeurs_donnees_aberrantes/nb_donnees,moyennes_ecarts_angulaires_D_perp_3,'r-.','LineWidth',2);
lg = legend('$D_\perp$ moyenne quadratique','$D_\perp$ phi1', '$D_\perp$ phi2');
