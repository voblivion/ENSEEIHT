...
% Affichage de l'image décorélée :
figure('Name','Image décorélée','Position',[0,0,0.33*L,0.3*L]);
imagesc(J);
axis off;
axis equal;
colormap(gray);

% Affichage de l'histogramme de l'image décorélée :
histogramme = hist(J(:),[-I_max:I_max]);
figure('Name','Histogramme des niveaux de gris après décorélation','Position',[0.33*L,0,0.33*L,0.3*L]);
plot([-I_max:I_max],histogramme);
axis([-I_max I_max 0 1.1*max(histogramme)]);
xlabel('Niveau de gris');
ylabel('Frequence');

% Vecteurs contenant les niveaux de gris de gauche (de droite) d'une paire de pixels :
% A DECOMMENTER/COMPLETER
I_gauche = J(1:1:size(J,1),1:1:size(J,2)-1);
I_gauche = I_gauche(:);			% Vectorisation de la matrice I_gauche
I_droite = J(1:1:size(J,1),2:1:size(J,2));
I_droite = I_droite(:);
